...
const handleConfirmDelete = () => {
  if (props.menuItem) {
    emit('confirm-delete', props.menuItem.item_Id)
  }
  emit('update:isOpen', false)
}
</script>
    ...
    <DialogContent class="max-w-md w-full h-auto p-0 bg-transparent border-0 shadow-none">
      ...
      >
        <div class="w-full flex flex-col justify-center items-center gap-6">
          <!-- Warning Icon -->
          <div
            class="w-11 h-11 p-2 bg-gray-200 rounded-full shadow-[inset_-4px_-4px_16px_0px_rgba(255,255,255,1.00)] shadow-[inset_8px_8px_16px_0px_rgba(190,190,190,1.00)] flex justify-center items-center"
          >
            <TriangleAlert class="w-5 h-5 text-red-400" />
          </div>

          <!-- Title -->
          <div class="text-center text-gray-800 text-base font-bold leading-tight">
            Delete Menu Item
          </div>
          ...
          <div class="flex justify-center items-center gap-3.5">
            ...
            >
              <div class="text-center text-gray-800 text-sm font-semibold leading-tight">
                Cancel
              </div>
            </button>
