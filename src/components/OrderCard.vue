    ...
    <div class="flex flex-col gap-5">
      <div class="flex flex-col gap-3.5">
        ...
        <div class="flex justify-between items-center">
          <div class="text-gray-800 text-base font-bold leading-normal">
        ...
        <div class="flex justify-between items-start">
          ...
          <div class="flex items-center gap-3.5 flex-wrap">
            <StatusButton
              ...
              :key="status.id"
              :is-next="order.status_Id! + 1 === status.id"
              :status="status.name.toLowerCase()"
              :icon="status.icon"
              :is-active="order.status_Id === status.id"
              @click="$emit('status-change', order.order_Id!, status.id)"
            />
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-2">
        ...
        >
          <div class="flex items-center gap-2.5">
            <span class="text-gray-700 text-base font-normal leading-normal">{{
              menu.items.find((i) => i.item_Id === item.item_Id)?.name
            }}</span>
            <span class="text-gray-700 text-base font-semibold leading-normal"
              >x{{ item.quantity }}</span
            >
          </div>
          <div class="text-gray-800 text-base font-normal leading-loose">
            ₱{{ menu.items.find((i) => i.item_Id === item.item_Id)?.price }}
          </div>
        </div>
      </div>
    </div>
