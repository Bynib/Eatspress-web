...
<template>
  <Card class="w-full shadow-[-6px_-6px_12px_#ffffff,6px_6px_12px_#BEBEBE]">
    <CardHeader>
      ...
      <CardDescription>Order placed on {{ new Date(item.created_At!).toLocaleString('en-US', {
        month: 'long',
        day: 'numeric',
        year: 'numeric',
        hour: 'numeric',
    ...
    <CardContent class="flex justify-between">
      <div class="text-sm lg:text-md gap-3 w-full flex flex-col rounded-2xl">
        <div class="w-full px-0 sm:px-0 md:px-5 flex justify-between">
          <div v-for="status in statuses" :key="status.id" class="flex flex-col justify-center items-center">
            <Button class=" p-2 rounded-full shadow-[-6px_-6px_12px_#ffffff,6px_6px_12px_#BEBEBE]">
              <component :is="status.icon" class="size-5" :class="status.id === item.status_Id ? 'text-[#FF6B6B]': 'text-gray-600'" />
            </Button>
            <p>{{ status.name }}</p>
          </div>
        </div>
        ...
        <div v-for="(item, index) in item.details" :key="index" class="w-full flex gap-3 justify-between">
          <p>
            {{menu.items.find((i) => i.item_Id === item.item_Id)?.name}} x {{ item.quantity }}
          </p>
          <p>₱{{item.quantity * (menu.items.find((i) => i.item_Id === item.item_Id)?.price ?? 0)}}</p>
        </div>

        <p class="font-semibold">Special Instructions:</p>
        <Textarea disabled :value="item.instruction"
          ...
          placeholder="No special instructions."></Textarea>
      </div>
    </CardContent>
