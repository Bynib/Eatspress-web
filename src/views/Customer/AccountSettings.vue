      ...
      <div
        ...
        <div class="flex flex-col lg:flex-row gap-5">
          <div class="lg:w-1/2">
            ...
            <Input :disabled="!editProfile" type="text" v-model="user.firstname"
          ...
          <div class="lg:w-1/2">
            ...
            <Input :disabled="!editProfile" type="text" v-model="user.lastname"
        ...
        <div class="flex flex-col lg:flex-row gap-5">
          <div class="lg:w-1/2">
            ...
            <Input :disabled="!editProfile" type="text" v-model="user.email"
              class="w-full p-2 rounded-lg border-none hover:border-none focus-visible:ring-0 bg-white shadow-[inset_0px_0px_0px_#ffffff,inset_3px_3px_5px_#BEBEBE]" />
          </div>
          <div class="lg:w-1/2">
            <p class="text-sm sm:text-sm md:text-md">Phone Number</p>
            <Input :disabled="!editProfile" type="text" v-model="user.phone_No"
              class="w-full p-2 rounded-lg border-none hover:border-none focus-visible:ring-0 bg-white shadow-[inset_0px_0px_0px_#ffffff,inset_3px_3px_5px_#BEBEBE]" />
          </div>
        </div>
        ...
        <div v-else class="flex flex-col lg:flex-row gap-5">
          <div class="lg:w-1/2">
            ...
            <div
              ...
              <Input :disabled="!editProfile" :type="showCurrentPassword ? 'text' : 'password'"
                v-model="user.oldPassword" class="border-none shadow-none hover:border-none focus-visible:ring-0" />
              <Eye v-if="showCurrentPassword" @click="togglePassword('current')"
                class="text-[#94A3B8] cursor-pointer" />
              <EyeClosed v-else @click="togglePassword('current')" class="text-[#94A3B8] cursor-pointer" />
            </div>
          </div>
          <div class="lg:w-1/2">
            ...
            <div
              class="w-full flex items-center gap-2 pr-2 rounded-lg bg-white shadow-[inset_0px_0px_0px_#ffffff,inset_3px_3px_5px_#BEBEBE]">
              <Input :disabled="!editProfile" :type="showNewPassword ? 'text' : 'password'" v-model="user.newPassword"
                class="border-none shadow-none hover:border-none focus-visible:ring-0" />
              <Eye v-if="showNewPassword" @click="togglePassword('new')" class="text-[#94A3B8] cursor-pointer" />
              <EyeClosed v-else @click="togglePassword('new')" class="text-[#94A3B8] cursor-pointer" />
            </div>
          </div>
          <div class="lg:w-1/2">
            ...
            <div
              class="w-full flex items-center gap-2 pr-2 rounded-lg bg-white shadow-[inset_0px_0px_0px_#ffffff,inset_3px_3px_5px_#BEBEBE]">
              <Input :disabled="!editProfile" :type="showConfirmPassword ? 'text' : 'password'"
                v-model="user.confirmPassword" class="border-none shadow-none hover:border-none focus-visible:ring-0" />
              <Eye v-if="showConfirmPassword" @click="togglePassword('confirm')"
                class="text-[#94A3B8] cursor-pointer" />
              <EyeClosed v-else @click="togglePassword('confirm')" class="text-[#94A3B8] cursor-pointer" />
            </div>
          </div>
        </div>
        <div class="flex justify-between w-full mt-5">
          ...
          <Button v-if="editProfile" @click="toggleButtons('add')"
        ...
        <div class="flex w-full gap-3">
          <!-- No address -->
          <Button v-if="addressStore.addresses.filter(r => !r.isDeleted).length === 0" @click="showDialog = true"
          ...
          <div v-else class="w-full">
            ...
            <div v-for="a in addressStore.addresses.filter(r => !r.isDeleted)" :key="a.address_Id"
              ...
              <Label :for="a.address_Id!.toString()" class="py-2" :class="!editProfile ? 'text-black/50' : ''">
                {{ a.unit_No }}, {{ a.street }}, {{ a.barangay }}, {{ a.city }}, {{ a.zip_Code }}
              </Label>
              <div class="space-x-2">
                <Button v-if="editProfile" @click="editAddress(a)"
                  class="text-[#FF6B6B] p-3 cursor-pointer rounded-full shadow-[-6px_-6px_12px_#ffffff,6px_6px_12px_#BEBEBE]">
                  <Pencil class="size-5" />
                </Button>
                <Button v-if="editProfile" @click="deleteAddress(a)"
        ...
        <Button :disabled="auth.isLoading" @click="toggleButtons('edit')"
        ...
        <Button v-if="editProfile" :disabled="auth.isLoading" @click="updateProfile"
    ...
    <DialogContent class="sm:max-w-md">
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="space-y-2">
          ...
          <Input id="zip_Code" v-model.number="newAddr.zip_Code" placeholder="ZIP Code" type="number" />
        </div>
      </div>
      <DialogFooter>
        <Button  :disabled="addressStore.isLoading" @click="addAddress" class="bg-gradient-to-r from-rose-500 to-orange-500 text-white">
          Save Address
        </Button>
      </DialogFooter>
    </DialogContent>
  ...
  <Dialog v-model:open="showEditDialog">
    <DialogContent class="sm:max-w-md">
      ...
      <div class="flex flex-col gap-3">
        ...
        <div class="space-y-2">
          <Label for="zip_Code">ZIP Code</Label>
          <Input id="zip_Code" v-model.number="editAddr.zip_Code" placeholder="ZIP Code" type="number" />
        </div>
      </div>
      <DialogFooter>
        <Button :disabled="addressStore.isLoading" @click="async () => await updateAddress(editAddr)"
